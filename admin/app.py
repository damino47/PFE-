...
def historique():
    ...
    cursor.execute("""
        SELECT 
            ...
            COUNT(CASE WHEN heure_sortie IS NULL THEN 1 END) as vehicules_presents,
            COUNT(CASE WHEN DATE(heure_entree) = CURDATE() THEN 1 END) as vehicules_aujourd_hui
        FROM sessions
    """)
    stats = cursor.fetchone()
...
def get_historique():
    try:
        ...
        query = '''
            SELECT 
                s.id,
                s.immatriculation as plaque,
                s.place_numero as place,
                s.heure_entree,
                s.heure_sortie,
                s.duree,
                s.montant as montant_total,
                COALESCE(p.montant_paye, 0) as montant_paye,
                COALESCE(p.montant_change, 0) as montant_change,
                COALESCE(p.status_paiement, 'en_attente') as status_paiement,
                CASE 
                    WHEN s.heure_sortie IS NULL THEN 'en_cours'
                    WHEN p.status_paiement = 'completed' THEN 'terminé'
                    ELSE 'en_attente'
        ...
        if date_debut and date_fin:
            query += " AND DATE(s.heure_entree) BETWEEN %s AND %s"
            params.extend([date_debut, date_fin])
        
        query += " ORDER BY s.heure_entree DESC"
        ...
        stats_query = '''
            SELECT 
                COUNT(*) as total_vehicules,
                COALESCE(SUM(s.montant), 0) as revenu_total,
                COUNT(CASE WHEN s.heure_sortie IS NULL THEN 1 END) as vehicules_presents,
                COUNT(CASE WHEN DATE(s.heure_entree) = CURDATE() THEN 1 END) as vehicules_aujourd_hui
            FROM sessions s
            WHERE 1=1
        '''
        
        if date_debut and date_fin:
            stats_query += " AND DATE(s.heure_entree) BETWEEN %s AND %s"
